...
const Player = ({ msg, settrfsong , onClick}) => {
  ...
  return (
    <div className="myDiv  md:p-2 bg-black/70 h-[80vh] p-2 w-full lg:w-[82%] top-12   lg:block  fixed z-0 md:top-[50%] md:rounded-xl lg:h-1/2 md:bottom-0 text-white overflow-y-scroll">
      <h2 className="text-xl lg:sticky lg:-top-5 -top-1 bg-black-60 lg:bg-black">Relative Results</h2>
      <ul className="space-y-3">
        {msg.map((song, index) => (
          <li
            key={song.id || index}
            className="flex items-center text-[10px] gap-4 lg:p-2 py-1 px-2 bg-[#161616] cursor-pointer rounded-lg"
          >
            <img
