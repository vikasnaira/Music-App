...
const Player = ({ msg, settrfsong , onClick}) => {
  if (!msg || msg.length === 0)
    return (
      <div className="bg-black/60 absolute bottom-0 md:h-[55%] h-[53vh] w-full flex items-center rounded-xl justify-center md:text-5xl text-amber-50">
        No songs found
      </div>
    );
    ...
    <div className="myDiv md:p-5 p-2 w-full fixed z-9 rounded-xl h-1/2  bottom-0 bg-black text-white overflow-y-scroll">
      ...
      <ul className="space-y-3">
        {msg.map((song, index) => (
          <li
            key={song.id || index}
            className="flex items-center text-[10px] gap-4 md:p-2 bg-[#161616] cursor-pointer rounded-lg"
          >
            <img
            ...
            <div className="flex w-[30%] justify-between gap-10">
              ...
              <p className="text-sm md:flex hidden">{song.playCount} Plays</p>
            </div>
            <button
